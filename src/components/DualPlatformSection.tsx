...
const DualPlatformSection: React.FC = () => {
  ...
  const stats = [
    { icon: <Zap className="h-6 w-6 text-yellow-500" />, label: "3 Hour Setup", value: "Lightning Fast" },
    { icon: <Users className="h-6 w-6 text-green-500" />, label: "500+ Clients", value: "Trusted Platform" },
    { icon: <TrendingUp className="h-6 w-6 text-blue-500" />, label: "98% Uptime", value: "Reliable Service" },
    { icon: <Shield className="h-6 w-6 text-red-500" />, label: "Secure", value: "Enterprise Grade" }
  ];
          ...
          {platformTypes.map((platform, index) => (
            <Card key={index} className="bg-white border-0 shadow-lg hover:shadow-xl transition-all duration-300 group">
              <CardContent className="p-6 lg:p-8">
                ...
                <Button 
                  className={`w-full bg-${platform.color}-600 hover:bg-${platform.color}-700 text-white group-hover:scale-105 transition-all duration-200`}
                  size="lg"
                >
                  {platform.ctaText}
